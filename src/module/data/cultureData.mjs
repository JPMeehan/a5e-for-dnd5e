...
export default class CultureData extends dnd5e.dataModels.SystemDataModel.mixin(
  ...
  /**
   * Set the culture reference in actor data.
  ...
  _onCreate(data, options, userId) {
    if (game.user.id !== userId || this.parent.actor?.type !== 'character')
      return;
    this.parent.actor.setFlag(moduleID, 'culture', this.parent.id);
  }

  /* -------------------------------------------- */

  /**
   * Remove the culture reference in actor data.
  ...
  async _preDelete(options, user) {
    ...
    await this.parent.actor.unsetFlag(moduleID, 'culture');
  }
}
